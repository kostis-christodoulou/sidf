...
# how many babies are born with a lowbirthweight of< 2500grams ?
smoking_birth_weight %>% 
  count(lowbirthweight, sort=TRUE) %>% 
  mutate(proportion = n/sum(n))
...
model4 <- lm(weight_kg ~ weeks + gender + habit + mother_age, data= smoking_birth_weight)
msummary(model4)

model5 <- lm(weight_kg ~ weeks + gender + habit + father_age+ mother_age + whitemom + visits + gained_kg, data= smoking_birth_weight)
msummary(model5)
